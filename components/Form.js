...
function Form() {
  ...
  return (
    <form className="flex flex-col relative space-y-2 text-black/80 dark:text-white/75">
      ...
      <button className="absolute bottom-0 right-0 bg-blue-400 hover:bg-blue-500 disabled:text-black/40 disabled:bg-white/75 disabed:cursor-not-allowed text-white rounded-full px-3.5 py-1" type="submit" onClick={uploadPost} disabled={!input.trim() && !photoUrl.trim()}>Post</button>
    </form>
  )
}
